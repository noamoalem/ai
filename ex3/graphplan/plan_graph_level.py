...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for action in all_actions:
            ...
            if previous_proposition_layer.all_preconds_in_layer(action):
                lst_pre = action.get_pre()
                for i in range(len(lst_pre)):
                    j = i+1
                    for j in range(len(lst_pre)):
                        if previous_proposition_layer.is_mutex(lst_pre[i], lst_pre[j]):
                            flag = False
                if flag:
    ...
    def update_proposition_layer(self):
        ...
        for action in current_layer_actions:
            add_lst = action.get_add()
            for prop in add_lst:
                if prop in dict_of_props:
                    dict_of_props[prop].append(action)
                else:
                    dict_of_props[prop] = [action]
        for prop in dict_of_props:
            prop.set_producers(dict_of_props[prop])
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    for p in a1_pre_list:
        for q in a2_pre_list:
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    if (not prop1) == prop2:
        return True
    prop1_lst = prop1.get_producers()
    prop2_lst = prop2.get_producers()
